...
Page({
    ...
    var val = wx.getStorageSync('userInfo');
    let data = { userid: val.userid};
    let token = val.token;
    wx.request({
      method:'POST',
      url: 'https://2.honggps.com/getUserSig', //仅为示例，并非真实的接口地址
      data: {
        data,
        token
      },
      ...
      success(res) {
        ...
        let aa = res.data.retobj.userSig;
        let sdkAppID = 1400226439;
        var url = `../room/room?roomID=${self.data.roomNo}&template=${self.data.template}&sdkAppID=${sdkAppID}&userId=${val.userid}&userSig=${aa}`;
