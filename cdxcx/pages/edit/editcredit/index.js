...
Page({
        ...
        wx.uploadFile({
          ...
          success: function (res) {
            ...
            if (v == 'carOnePic') {
              ...
              that.setData({
                IdCardFront: host + m.retobj
              })
            } else if (v == 'carTwoPic') {
              that.setData({
                IdCardBack: host + m.retobj
              })
            } else if (v == 'carThreePic') {
              that.setData({
                NetworkLetterAuthorization: host + m.retobj
              })
            } else if (v == 'carFourPic') {
              that.setData({
                authorization: host + m.retobj
              })
            } else if (v == 'carFivePic') {
              that.setData({
                authorizationLetter: host + m.retobj
              })
            } else if (v == 'carSixPic') {
              that.setData({
  ...
  onLoad: function (e) {
    ...
    for (let i = 0; i < _self.data.creditType.length; i++) {
      if (_self.data.creditType[i] = data.creditType) {
        _self.setData({
          genderIndex :i
        })
      }
    ...
    this.setData({
      ...
      IDNumberAddress: data.IDNumberAddress,
      IDNumberIndate: data.IDNumberIndate,
      IdCardBack: data.IdCardBack,
      creditid: data.creditid,
      IdCardFront: data.IdCardFront,
      NetworkLetterAuthorization: data.NetworkLetterAuthorization,
  ...
  formSubmit: function (e) {
    ...
    let list = {
      ...
      creditid: that.data.creditid,
      NetworkLetterAuthorization: that.data.NetworkLetterAuthorization,
      authorization: that.data.authorization,
      authorizationLetter: that.data.authorizationLetter,
